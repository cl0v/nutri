@Skip('modifiquei tudo para o helper e controller')
...
main() {
  test('Title behavior after pressing nextDay HOJE > AMANHÃ', () {
    final homeTitleViewModel = HomeTitleHelper();
  ...
  group('dayIndex |', () {
    test(
      ...
      final homeTitleViewModel = HomeTitleHelper();
      homeTitleViewModel.model.todayIndex = 1;
      expect(homeTitleViewModel.model._showingDayIndex.stream,
    ...
    test(
      ...
      expect(
          homeTitleViewModel.model._showingDayIndex.stream,
          emitsInOrder([
            4,
            5,
            6,
  ...
  test('Testing backToToday method', () {
    HomeTitleHelper homeTitleViewModel = HomeTitleHelper();
    homeTitleViewModel.model.todayIndex = 1;
    homeTitleViewModel.nextDay();
    homeTitleViewModel.nextDay();
    expect(homeTitleViewModel.model.title.value, equals('QUARTA'));
    expect(homeTitleViewModel.model._showingDayIndex.value, 3);
    homeTitleViewModel.backToToday();
    expect(homeTitleViewModel.model.title.value, equals('HOJE'));
    expect(homeTitleViewModel.model._showingDayIndex.value, 1);
  });
  ...
  test('O botao de nextDay deve acrescentar um ao dia', () {
    ...
    homeTitleViewModel.model.todayIndex = 1;
    homeTitleViewModel.nextDay();
    expect(homeTitleViewModel.model._showingDayIndex.value, 2);
  }, skip: true);
  ...
  test('O dia nunca deve ser menor que 1, quando chega a 1, vira 7', () {
    ...
    homeTitleViewModel.model.todayIndex = 1;
    // homeTitleViewModel.previewDay()();
    expect(homeTitleViewModel.model._showingDayIndex.value, 7);
  }, skip: true);
}
